...
## Inclusion of Senf disturbance data in FUNDIV
...
```{targets plot-prevalence-per-country-per-year}
tar_target(plot_prevalence_per_country_per_year, 
           plot_annualPrevalence(disturbance_per_plot_200m, FUNDIV_tree))
```
...
```{targets plot-disturbedArea-perMortalityRate}
list(
...
```{targets plot-disturbedArea-perMortalityRate-different-death-cause}
list(
