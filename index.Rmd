...
### Format to Finnish NFI template

The Finnish NFI template is slightly different from the french template or FUNDIV template. Therefore, we need to format them. 
...
For each country, we create a dataset containing the dusturbance type ([Senf & Seidl, 2021a](https://doi.org/10.1111/gcb.15679)) and year ([Senf & Seidl, 2021b](https://www.nature.com/articles/s41893-020-00609-y)) that dominates in area a buffer of 15m around each FUNDIV plot center (if any disturbance occured). 

```{targets import-disturbance-Senf-majority}
list(
  tar_target(FUNDIV_Plots_disturbance_belgium, 
             Get_disturbance_majority_per_plot("belgium", 15, FUNDIV_tree,
                                      "data/Disturbance")), 
  tar_target(FUNDIV_Plots_disturbance_france, 
             Get_disturbance_majority_per_plot("france", 15, FUNDIV_tree,
                                      "data/Disturbance")), 
  tar_target(FUNDIV_Plots_disturbance_germany, 
             Get_disturbance_majority_per_plot("germany", 15, FUNDIV_tree,
                                      "data/Disturbance")), 
  tar_target(FUNDIV_Plots_disturbance_finland, 
             Get_disturbance_majority_per_plot("finland", 15, FUNDIV_tree,
                                      "data/Disturbance")), 
  tar_target(FUNDIV_Plots_disturbance_sweden, 
             Get_disturbance_majority_per_plot("sweden", 15, FUNDIV_tree,
                                      "data/Disturbance")), 
  tar_target(FUNDIV_Plots_disturbance_spain, 
             Get_disturbance_majority_per_plot("spain", 15, FUNDIV_tree,
                                      "data/Disturbance"))
)
```
...
```{targets merge-disturbance-plots-majority}
...
#### Area percentage approach

We use an alternative, more precise approach. For each plot, we extract the percentage of the buffer area covered by each type of disturbance that occured between 1986 and 2020.
...
Secondly, we add this information in the FUNDIV database by keeping only the distubrances that occured between the two census.
...
Lastly, we merge the datasets of each country to have the disturbance "with area" for all FUNDIV countries. 

```{targets merge-FUNDIV-with-disturbance-area}
tar_target(FUNDIV_tree_with_disturbance_area, 
           rbind.data.frame(FUNDIV_tree_with_diturbance_area_belgium,
                            FUNDIV_tree_with_diturbance_area_france, 
                            FUNDIV_tree_with_diturbance_area_germany, 
                            FUNDIV_tree_with_diturbance_area_finland, 
                            FUNDIV_tree_with_diturbance_area_sweden, 
                            FUNDIV_tree_with_diturbance_area_spain))
```
...
When enlarging the buffer around plot center to 100m-radius, we can plot the percentage of the buffer around each plots covered by the three types of disturbance. 

```{targets map-area-covered-by-disturbance}
tar_target(Map_areaCoveredPerDisturbance_FUNDIVplots, 
           Map_FUNDIVplots_areaCoveredPerDisturbance(FUNDIV_tree_with_disturbance_area))
```
...
```{targets Compare-NFI-Senf-disturbance-area}
tar_target(plot_Compare_NFI_Senf_disturbance_area, 
           Compare_NFI_Senf_disturbance_area(
